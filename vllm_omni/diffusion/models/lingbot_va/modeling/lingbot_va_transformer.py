...
"""LingBot-VA transformer port (single-GPU inference path)."""
...
for _module_name in ("flash_attn_interface", "flash_attn"):
    try:
        _module = import_module(_module_name)
        flash_attn_func = getattr(_module, "flash_attn_func")
        break
    except Exception:
        continue


def custom_sdpa(q: torch.Tensor, k: torch.Tensor, v: torch.Tensor):
...
class WanTimeTextImageEmbedding(nn.Module):
    ...
    def forward(self, timestep: torch.Tensor, dtype=None):
        batch_size, seq_len = timestep.shape
        ...
        return temb.reshape(batch_size, seq_len, -1), timestep_proj.reshape(batch_size, seq_len, -1)
...
class WanAttention(nn.Module):
    def __init__(
        ...
        if attn_mode == "torch":
            ...
            if flash_attn_func is None:
                raise RuntimeError("flash_attn is not available but attn_mode='flashattn' was requested")
    ...
    def clear_pred_cache(self, cache_name: str):
        if self.attn_caches is None or cache_name not in self.attn_caches or self.attn_caches[cache_name] is None:
            return
    ...
    def allocate_slots(self, cache_name: str, key_size: int):
        ...
        if free.numel() < key_size:
            raise RuntimeError("Unable to allocate KV cache slots")
    ...
    def update_cache(self, cache_name: str, key: torch.Tensor, value: torch.Tensor, is_pred: bool):
        cache = self.attn_caches[cache_name]
    ...
    def restore_cache(self, cache_name: str, slots: torch.Tensor):
...
class WanTransformer3DModel(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        self.rope = WanRotaryPosEmbed(attention_head_dim, self.patch_size, rope_max_seq_len)
        # Some LingBot checkpoints include both patch_embedding.* (Conv3d)
        # and patch_embedding_mlp.*. Keep both names for compatibility.
